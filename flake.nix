  ...
  outputs =
    ...
    in
    {
      ...
      homeConfigurations = {
        ...
        # Key is "root" (not "herdr") to match `$USER` on the box, so
        # `home-manager switch --flake .#$USER` (mise's `user` task) resolves.
        # Apply with: nix run home-manager/release-26.05 -- switch --flake github:fellnerse/nix-darwin#root
